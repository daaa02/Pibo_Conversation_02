...
from Pibo_Package_02.Pibo_Conversation.src.drive_upload import drive_upload
...
class RunSchedule():
    ...
    def day(self):
        ...
        #     logfile = f'(P02)_{datetime.now().strftime("%m%d_%H%M")}.txt'   #('P<PACKAGE_NUMBER>_DATE.txt')   
        ...
        try:
            # 두 번째 랜덤 활동
            self.act1 = f'Pibo_Play/src/Com/com_2.py'   
            folder1 = "/home/pi/UserData"
            logfile1 = f'(P02)_{datetime.now().strftime("%m%d_%H%M")}.txt'   #('P<PACKAGE_NUMBER>_DATE.txt')   
            f = open(f'{folder1}/{logfile1}','w')
            ...
            try:
                drive_upload(filepath=f'{folder1}/{logfile1}', filename=logfile1)
            ...
            # 헤어짐   
            self.act2 = 'Pibo_Conversation/src/goodbye.py'   
            folder2 = "/home/pi/UserData"
            logfile2 = f'(P02)_{datetime.now().strftime("%m%d_%H%M")}.txt'   #('P<PACKAGE_NUMBER>_DATE.txt')   
            f = open(f'{folder2}/{logfile2}','w')
            ...
            try:
                drive_upload(filepath=f'{folder2}/{logfile2}', filename=logfile2)
